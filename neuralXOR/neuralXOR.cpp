...
void random_weights(const int n, double w1[], double w2[], double w3[])
{
    ...
    {
        w1[i] = (5 - rand() % 10) * 0.1;     //Вес от первого входа до скрытого слоя
        w2[i] = (5 - rand() % 10) * 0.1;     //Вес от второго входа до скрытого слоя
        w3[i] = (5 - rand() % 10) * 0.1;     //Вес от скрытого слоя до выхода
    }
}
